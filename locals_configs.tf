locals {
  configs_merged_services_homepage = merge(
    {
      for k, server in local.outputs_servers : "1 - ${k} (${server.title})" => merge([
  ...
  configs_outputs = merge(
    ...
    },
    {
      for k, service in local.services_merged : k => {
        ...
        "/app/config/settings.yaml"   = templatefile("templates/${service.service}/settings.yaml", { default = var.default, homepage = service, services = local.configs_merged_services_homepage })
        "/app/config/services.yaml"   = templatefile("templates/${service.service}/services.yaml", { services = local.configs_merged_services_homepage })
